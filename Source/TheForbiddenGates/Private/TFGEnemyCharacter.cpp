...
void ATFGEnemyCharacter::BeginPlay()
{
    ...
    {
        ASC->GetGameplayAttributeValueChangeDelegate(UTFGAttributeSet::GetHealthAttribute()).AddUObject(this, &ATFGEnemyCharacter::HandleHealthChanged);
    }
}

void ATFGEnemyCharacter::HandleHealthChanged(const FOnAttributeChangeData& ChangeData)
{
    if (ChangeData.NewValue <= 0.0f) HandleDeath();
}
